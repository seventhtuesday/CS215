...
Original file is located at
    https://colab.research.google.com/drive/1L7YtDUqfjZhvraqbcu9-KEcFbtPb84i5
...
def gs1_error(n):
    est = gs1_method(n)
    error = np.linalg.norm(np.array([1, 1])-np.array(est))
    return error
...
def gs2_error(n):
    est = gs2_method(n)
    error = np.linalg.norm(np.array([2, 1])-np.array(est))
    return error
